define([], function() {
    ...
    var xy_select = {
        doClick: function(data) {
            ...
            // console.log($(this))
            $(".configbar").append(' <div id = "maskLayer" style = "display:none"><div id = "drag" ><div id = "drag_con" > </div> </div > </div>');
            var dragHtml = '<div id="FuntypeAlpha">'; //职能类别
            dragHtml += '<div id="FuntypeList"></div>'; //职能类别列表
            dragHtml += '</div>';
            $('#drag_con').html(dragHtml);
            xy_select.Show2(data);
            $('#maskLayer').css({ top: 0, right: 0 }).show();
