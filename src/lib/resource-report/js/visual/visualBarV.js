...
function getOptionBar1(data, tips){
    ...
    var colorCfg = [['#87BCFC', '#86BCFC'], ['#286FC8', '#286FC8'], ['#498FE6', '#498FE6'], ];
    var color = [GDC(colorCfg[0]), GDC(colorCfg[1]), GDC(colorCfg[2])];

    var series = data.map(function(d, i){
        return {
            name: d.name, type:'bar', stack: null,
            itemStyle:{normal:{ color: color[i]}}, label: {},
    ...
    var option = {
        tooltip: { trigger: 'item', textStyle:{ fontFamily:"宋体", fontSize:12 },
...
function getOptionBar2(data, tips){
    ...
    var colorCfg = [['#EAF3FE', '#ADD1FC'], ['#498FE6', '#2D74CD'], ['#286FC8', '#286FC8']] ;
    var color = [GDC(colorCfg[0]), GDC(colorCfg[1]), GDC(colorCfg[2])];

    var series = data.map(function(d, i){
        var label = {normal: {show: true, offset:[0, 10], position: 'insideTop', textStyle:{color:"#000"},
            formatter:function(d){ return ("P" + (d.seriesIndex + 1)*25) + "(" + d.data + ")"; }
        }};
        return {
            name: d.name, type:'bar', stack: true,
            itemStyle:{normal:{ color: color[i]}}, label: label,
            data: d.value.map(function (c) { return c.value; }),
        };
    });
...
function getWordCloudHtml(dt, color){
    var maxFontSize = 20, minFontSize = 8, maxLineWidth = 180, disF = maxFontSize - minFontSize;
    var padding = 10, color = color || "#fff";   //标签内右边距， 标签字体颜色
    ...
    var tpv = dt.map(function(d){ return d.value; }),
        maxV = Math.max.apply(null, tpv), minV = Math.min.apply(null, tpv), disV = maxV - minV;
    ...
    function sortMiddle(_dt){
        var temp = _dt.sort(function(a, b){ return b[1]-a[1]; }), rst = _dt.map(function(d){ return ["", 0]; });
    ...
    var html = "", lsLine = [{data:[], lineWidth:0}], j = 0;;
    for(var i=0; i<data.length; i++){
        var d = data[i], w = visualLength(d[0], d[1]);
    ...
    lsLine.forEach(function(line){
        var p = "", pw = parseInt((maxLineWidth-line.lineWidth)/2);
...
//需求量图
function ShowBarMoney(id, data, tips, callbackClick){
